...
function setActionBar() {
	...
	setTitle(RESERVATION_NOSTATUS_TITLE);
}
...
function setTitle(title) {
	...
	if(!mui.os.ipad) {
		plus.webview.getWebviewById(ReservationId).evalJS('setTitle("' + title + '")');
	} else {
...
function getDataList() {
	...
	var template = '<li id="consultIdTemplate" class="mui-table-view-cell mui-media">' +
		...
		'<p class="mui-ellipsis" style=\"text-indent:0px;\">手术部位: Drawn-template</p>' +
		'<p class="mui-ellipsis" style=\"text-indent:0px\">申请医院: location-template</p>' +
		'</div>' +
		'</a>' +
		'<div class ="mui-badge"' +
	...
	$.ajax({
		...
		success: function(response) {
			...
			switch(getParameter(statusKey)) {
				case '2':
					setTitle(NOT_RESERVATION_TITLE + '(' + response.dataCount + ')');
					break;
				case '5':
					setTitle(RESERVATION_TITLE + '(' + response.dataCount + ')');
					break;
				case '101':
					setTitle(BACK_RESERVATION_TITLE + '(' + response.dataCount + ')');
					break;
				default:
					setTitle(RESERVATION_NOSTATUS_TITLE + '(' + response.dataCount + ')');
					break;
			...
			if(parseInt(response.dataCount) == 0) {
				...
				for(var i = 0; i < dataList.length; i++) {
					...
					var status = decorStatus(dataList[i].Status);
					...
					contentContainer += template
						...
						.replace(/details-template/, dataList[i].YuName + '-' +
							dataList[i].YuSex + '-' + dataList[i].YuAge)
						.replace(/Drawn-template/, dataList[i].OperationPart)
						.replace(/location-template/, dataList[i].CenterName)
...
function setItemClickListener() {
	...
	$('#container a').on('tap', function() {
		...
		openWebView('reservationDetails.html?consultId=' + consultId + '&status=' + status +
