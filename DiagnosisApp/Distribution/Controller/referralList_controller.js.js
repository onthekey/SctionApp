...
function setTitle(title) {
	...
	if(!mui.os.ipad) {
		plus.webview.getWebviewById(ReferralId).evalJS('setTitle("' + title + '")');
	} else {
...
function getDataList() {
	...
	$.ajax({
		...
		success: function(response) {
			...
			if(parseInt(response.currentCount) <= 20)
				$('#container').empty();
			$('#container').append(contentContainer);
...
function setItemClickListener() {
	...
	$('#container a').on('tap', function() {
		...
		console.log('diagnosisDetails.html?consultId=' + consultId + '&status=' + status 
		+ "&DiagHisIp=" + $(this)[0].getAttribute('data-DiagHisIp') 
		+ "&DiagHisPort=" + $(this)[0].getAttribute('data-DiagHisPort'),
			DiagnosisDetailsId);
		openWebView('diagnosisDetails.html?consultId=' + consultId + '&status=' + status 
		+ "&DiagHisIp=" + $(this)[0].getAttribute('data-DiagHisIp') 
		+ "&DiagHisPort=" + $(this)[0].getAttribute('data-DiagHisPort'),
			DiagnosisDetailsId);
	})
