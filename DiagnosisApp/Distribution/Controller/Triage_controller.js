...
mui.init({
	...
	subpages: [{
		url: 'TriageList.html?status=' + getParameterNoPlus('status'),
		id: TriageListId,
		styles: {
			top: '45px',
			bottom: '0px',
...
// 返回按钮监听
mui.back = function() {
	console.log('Triage_controller:back listener');
...
function dialogShow() {
	if(navigator.userAgent.indexOf("Html5Plus") < 0) { //不支持5+ API
		window.frames[TriageListId].close();
	} else { //支持5+ API
		if(contentWebview == null) {
			contentWebview = plus.webview.currentWebview().children()[0];
		}
