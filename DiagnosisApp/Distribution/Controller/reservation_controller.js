...
mui.init({
	...
	subpages: [{
		url: 'reservationList.html?status=' + getParameterNoPlus('status'),
...
// 返回按钮监听
mui.back = function() {
	console.log('Reservation: back listener');
	if(navigator.userAgent.indexOf("Html5Plus") < 0) { //不支持5+ API
		...
		plus.webview.close(ReservationId);
	}
};
...
function dialogShow() {
	if(navigator.userAgent.indexOf("Html5Plus") < 0) { //不支持5+ API
		window.frames[ReservationListId].close();
	} else { //支持5+ API
		if(contentWebview == null) {
			contentWebview = plus.webview.currentWebview().children()[0];
		}
